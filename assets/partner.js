    ...
    for (k=0; k<2; k++){
        ...
        for (i=0;i<patner.length;i++){
            ...
            for (j=1;j<patner[i].length;j++){
                document.write('<div class="gallery-grid-column"> \
                    <div class="gallery-grid-item h-40 w-40 bg-gradient-to-t from-gray-100 to-gray-50 border border-gray-100 relative flex items-center"> \
                        <div class="gallery-img-wrapper w-full p-5"> \
                            <img class="max-h-16 mx-auto" loading="lazy" width="200" height="200" src="', patner[i][j].src,'" alt="', patner[i][j].alt,'" title="', patner[i][j].title,'"/>\
                            <div class="sr-only">',patner[i][j].nom ,'</div> \
                        </div>\
                        <div class="absolute -bottom-3 -left-3 h-6 w-6 bg-gradient-to-t z-10"\
                            data-category="',patner[i][j].cat ,'"> </div> \
                    </div> \
                </div>');
            }
            document.write('</div>\n');
